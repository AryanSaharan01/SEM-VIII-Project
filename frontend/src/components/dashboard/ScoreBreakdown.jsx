...
const ScoreBreakdown = ({ skill, data, sessions }) => {
    ...
    return (
      <div className="glass-card glass-glow rounded-2xl p-6">
        <div className="flex flex-col items-center justify-center h-64">
          ...
          <p className="text-gray-400">Loading score breakdown...</p>
        </div>
      </div>
    )
  }
  ...
  const insights = useMemo(() => {
    ...
    const getTimestamp = (s) => s.clientTs || s.client_ts
    const getDuration = (s) => s.durationSeconds || s.duration_seconds || 0

    const lastWeekSessions = safeSessions.filter(s => new Date(getTimestamp(s)) >= lastWeek)
    const prevWeekSessions = safeSessions.filter(s => 
      new Date(getTimestamp(s)) >= twoWeeksAgo && new Date(getTimestamp(s)) < lastWeek
    )

    const lastWeekTime = lastWeekSessions.reduce((sum, s) => sum + getDuration(s), 0)
    const prevWeekTime = prevWeekSessions.reduce((sum, s) => sum + getDuration(s), 0)
    ...
    for (let i = 3; i >= 0; i--) {
      ...
      const weekSessions = safeSessions.filter(s => {
        const date = new Date(getTimestamp(s))
        return date >= weekStart && date < weekEnd
      })
      weeklyActivity.push({
        week: `W${4-i}`,
        sessions: weekSessions.length,
        hours: weekSessions.reduce((sum, s) => sum + getDuration(s), 0) / 3600
      })
    }
    ...
    return {
      ...
      avgSessionLength: safeSessions.length > 0 
        ? safeSessions.reduce((sum, s) => sum + getDuration(s), 0) / safeSessions.length / 60 
        : 0
    }
  }, [safeSessions])
  ...
  const chartData = Object.entries(data).map(([key, value]) => ({
    ...
    weightedScore: ((value.score || 0) * (value.weight || 0)) / 100
  }))
  ...
  // Use skill.score from DB as the authoritative score — it's the single source of truth
  const displayScore = skill?.score ?? Math.round(totalWeightedScore)

  // Get consistency score safely
  const consistencyScore = data?.consistency?.score || skill?.consistencyScore || skill?.consistency_score || 0

  return (
    <div className="space-y-6">
      {/* Overall Score */}
      <div className="glass-card glass-glow rounded-2xl p-8 border-primary-500/15">
        <div className="text-center">
          ...
          >
            <div className="text-7xl font-bold bg-gradient-to-r from-primary-400 to-purple-400 bg-clip-text text-transparent mb-3">
              {displayScore}
            </div>
            <div className="text-gray-300 font-semibold text-lg mb-4">Authenticity Score</div>
            <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-primary-600 to-purple-600 text-white px-8 py-3 rounded-2xl font-semibold shadow-lg shadow-primary-500/20">
              <Award className="w-5 h-5" />
              <span>{displayScore >= 75 ? 'Established' : displayScore >= 50 ? 'Evolving' : 'Tentative'}</span>
            </div>
          </motion.div>
        </div>
      </div>
        ...
        <div className="glass-card glass-glow rounded-2xl p-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <motion.div
              ...
              className="glass rounded-2xl p-4 border border-blue-500/15 hover:border-blue-500/25 transition-all duration-300"
            >
              ...
              <div className="text-xs text-gray-400 mb-2">Learning Velocity (Last Week)</div>
            ...
            <motion.div
              ...
              className="glass rounded-2xl p-4 border border-green-500/15 hover:border-green-500/25 transition-all duration-300"
            >
              <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-xs text-gray-500">
                {insights.weeklyActivity[insights.weeklyActivity.length - 1]?.sessions || 0} sessions this week
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              className="glass rounded-2xl p-4 border border-purple-500/15 hover:border-purple-500/25 transition-all duration-300"
            >
              <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-xs text-gray-400 mb-2">Topics Revisited</div>
              <div className="text-xs text-gray-500">
            ...
            <motion.div
              ...
              className="glass rounded-2xl p-4 border border-orange-500/15 hover:border-orange-500/25 transition-all duration-300"
            >
              <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-xs text-gray-400 mb-2">Avg Session Length</div>
              <div className="text-xs text-gray-500">
          ...
          <div className="mb-6">
            <h4 className="text-sm font-semibold text-gray-300 mb-3">4-Week Activity Trend</h4>
          ...
          {insights.revisitedTopics.length > 0 && (
            <div>
              <h4 className="text-sm font-semibold text-gray-300 mb-3">Most Revisited Topics</h4>
              <div className="space-y-2">
                {insights.revisitedTopics.slice(0, 5).map(([topic, count]) => (
                  <div key={topic} className="flex items-center justify-between bg-white/5 rounded-lg px-4 py-2">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        {/* Bar Chart */}
        <div className="glass-card glass-glow rounded-2xl p-6">
          <h3 className="text-xl font-bold text-white mb-6">Component Breakdown</h3>
        ...
        {/* Radar Chart */}
        <div className="glass-card glass-glow rounded-2xl p-6">
          <h3 className="text-xl font-bold text-white mb-6">Skill Profile Radar</h3>
          <ResponsiveContainer width="100%" height={300}>
      ...
      {/* Detailed Breakdown */}
      <div className="glass-card glass-glow rounded-2xl p-6">
        <h3 className="text-xl font-bold text-white mb-6">Detailed Component Analysis</h3>
          ...
          {Object.entries(data).map(([key, value], index) => {
            ...
            return (
              <motion.div
                ...
                className="border border-white/[0.06] rounded-2xl p-4 hover:border-white/10 hover:shadow-lg hover:shadow-black/20 transition-all duration-300 glass"
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <h4 className="font-semibold text-white mb-1">{capitalizedName}</h4>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div className="text-sm font-bold text-gray-300 min-w-[60px] text-right">
      ...
      <div className="glass rounded-2xl p-6 border border-blue-500/15">
        <div className="flex items-start space-x-3">
          ...
          <div>
            <h4 className="font-semibold text-white mb-2">How Scores Are Calculated</h4>
